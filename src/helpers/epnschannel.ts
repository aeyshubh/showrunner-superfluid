...
export class EPNSChannel {
  ...
  async superFluidData(walletAddress){
...
const sf = await Framework.create({
  ...
  private async getWalletKey(dirname: string) {
    ...
    this.logInfo('currentWalletInfo: %o', currentWalletInfo);
     //let a =  await this.superFluidData();
    // this.logInfo(`The Value is ${a}`);
    ...
    let walletKeyMeta = {
      ...
      chainId: isOldStandard ? `eip155:5` : walletInfo.CHAIN_ID,
    };
  ...
  async init() {
    ...
    try {
      ...
      this.logInfo(`Channel Sub : ${await this.getChannelSubscribers()}`);
